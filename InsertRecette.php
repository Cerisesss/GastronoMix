...
<?php
    ...
    $query_verif_categorie = "SELECT * FROM categorie;";
    $result_verif_categorie = $mysqli->query($query_verif_categorie);

    if($result_verif_categorie->num_rows > 0) {
        echo "Les categories existent déjà dans la database\n";
    } else {
    ...
    $query_verif_recette = "SELECT * FROM recette;";
    $result_verif_recette = $mysqli->query($query_verif_recette);

    if($result_verif_recette->num_rows > 0) {
        echo "Les recettes existent déjà dans la database\n";
    } else {
        foreach($recetteJson as $index_nom_json => $nom_json) {
            foreach($nom_json as $index_recette => $recette) {
                ...
                foreach($recette->ingredients as $key_ingredient => $ingredient) {
                    foreach($recette->ingredients_recherche as $ingredient_recherche) {
                        ...
                        if($result_ingredient == false && $ingredient != "") {
                ...
                foreach($recette->quantite as $key_quantite => $quantite) {
                    ...
                    //recupere l'id_unite selon l'indice de la quantite/unite/ingredient (c'est la meme)
                    $unite = $recette->unite[$key_quantite];
                    $query_id_unite = "SELECT id_unite FROM unite where libelle_unite = \"$unite\";";
                    $result_id_unite = $mysqli->query($query_id_unite);
            ...
            $compteur++;
            echo "ajout avec succes : " . $compteur . "/4 \n" ;
        }
    }
